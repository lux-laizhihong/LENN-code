...
local_area_center_x1 = x_crackTip1+0.1
local_area_center_y1= y_crackTip1
local_area = 0.5
scalar_1 = 1/local_area
translate_x1 = -1*local_area_center_x1/local_area
translate_y1 = -1*local_area_center_y1/local_area
kernel_1 = soft_multidomain.cubic_BSpline_kernel(local_area_center_x1, local_area_center_y1, local_area)

crack_net1 = Enrichment.extendAxisNet(
        net = AxisScalar2D(
...
plate_center_1 = soft_multidomain.weighted_DEM(model=crack_net1,
                                            weight=kernel_1)
...
local_area_center_x2 = x_crackTip2-0.1
local_area_center_y2= y_crackTip2
local_area = 0.5
scalar_2 = 1/local_area
translate_x2 = -1*local_area_center_x2/local_area
translate_y2 = -1*local_area_center_y2/local_area
kernel_2 = soft_multidomain.cubic_BSpline_kernel(local_area_center_x2, local_area_center_y2, local_area)

crack_net2 = Enrichment.extendAxisNet(
        net = AxisScalar2D(
            stack_net(input=3,output=2,activation=nn.Tanh,width=width,depth=depth),
            A=torch.tensor([scalar_2,scalar_2,1.0]),
            B=torch.tensor([translate_x2,translate_y2,0.0])
            ),
        extendAxis= crack_extension_2)

plate_center_2 = soft_multidomain.weighted_DEM(model=crack_net2,
                                            weight=kernel_2)

pinn = Plate([plate_whole,plate_center_1,plate_center_2])
pinn.add_BCPoints()
...
a = 2
model_name = 'crack_extension-two-edgecrack-domain-step0-disload'

pinn.train(path=model_name, patience=300, epochs=25000, eval_sep=100)
# pinn.load(path='C:/Users/15844/PycharmProjects/pythonProject4/DEM/crack_extension-two-edgecrack-domain-step0-disload')
